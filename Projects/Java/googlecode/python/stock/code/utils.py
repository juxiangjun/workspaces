...
class StockIndexParser(HTMLParser):
	selected=('strong')
	def __init__ (self):
	...
	def	reset(self):
		...
		self._level_stack = []
	def handle_starttag(self, tag, attrs):
		if tag in StockIndexParser.selected: 
			self._level_stack.append(tag)

	def handle_endtag(self, tag):
		if self._level_stack and tag in StockIndexParser.selected and tag == self._level_stack[-1]:
			self._level_stack.pop()

	def handle_data(self, data):
		if "/".join(self._level_stack) in ('tr/tr/tr/tr/strong','table/tr/td') and data != '\n':
...
class StockHTMLParser(HTMLParser):
	...
	def	reset(self):
		...
		self._level_stack = []
	def handle_starttag(self, tag, attrs):
		if tag in StockHTMLParser.selected: 
			for name, value in attrs:
				#print name + '\n'
				if name =='bgcolor':
					self._level_stack.append(tag)
	...
	def handle_endtag(self, tag):
		if self._level_stack and tag in StockHTMLParser.selected and tag == self._level_stack[-1]:
			self._level_stack.pop()
	def handle_data(self, data):
		if "/".join(self._level_stack) in ('table/td','table/tr/td') and data != '\n':
			self._lines.append(data)
...
class MySQLAdapter:
	...
	def run(self, sql_file):
		cmd = 'mysql -h '+self.host+' -u '+self.user+' -p'+self.password+'  '+self.db_name +'<'+sql_file
		os.system(cmd)
