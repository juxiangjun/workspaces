...
class Profile(Base):
	...
	def __init__(self, stock, data):
		...
		self.en_name = get_data(data[5])
		self.registration_addr = get_data(data[6])
		self.addr = get_data(data[7])
		self.principal = get_data(data[8])
		self.board_secretary = get_data(data[9])
		self.email = get_data(data[10])
		self.tel = get_data(data[11])
		self.web_site = get_data(data[12])
		self.industry = get_data(data[13])
		self.sse_industry = get_data(data[14])
		self.province = get_data(data[15])
		self.ab_status = get_data(data[16])
		self.is_sample = get_data(data[17])
		self.overseas_listing = get_data(data[18])
		self.overseas_addr = get_data(data[19])
