...
public class PrdRecommendationDAOImpl extends HibernateDAO<PrdRecommendation, Integer> implements PrdRecommendationDAO {
	...
	@Override
	public List<PrdRecommendation> findByTopNumber(Integer[] typeId, int number) {
		
		String hql = "from PrdRecommendation where 1=1 ";
		String sql="";
		for(Integer i=0;i<typeId.length;i++){
			if(i.intValue() ==0){
				sql +=" (typeId = "+typeId[i];
			}else{
				sql += " or typeId = "+typeId[i];
			}
		}
		hql += sql +") and status=1 order by sequence";
		return this.findTopRows(hql, number, "typeId", typeId);
	}
}
