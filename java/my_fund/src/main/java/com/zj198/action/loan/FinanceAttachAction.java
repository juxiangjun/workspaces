...
public class FinanceAttachAction extends BaseAction {
	...
	public String downTemplate(){
		attach = financeApplyService.getOrdFinanceApplyAttach(attachId);
		if(attach.getAttachType() == 1){
			prdTemplateList = financeAttachService.findOrdSupplyTemplate(attachId);
		}else{
			prdTemplateList = financeAttachService.findPrdAttachTemplate(attach.getFinanceDataId());
		}
		return "viewTemplate";
	}
