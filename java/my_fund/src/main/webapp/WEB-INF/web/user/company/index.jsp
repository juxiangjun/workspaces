...
<div class="container_960">
  <div class="user_box">
    ...
    <div class="user_real fl">
      ...
      <s:if test="#session._user.activetype==0 || #session._user.activetype==1"><p>您的手机尚未通过验证，为了您更方便开展业务，<a href="/user/regInfo.act"><font style="color: red;">请验证</font></a>您的手机。</p></s:if>
            <p>
            	<s:if test="#session._user.viptype == 0"><a href="/user/payment.act" class="btn_s bred">成为通资信通会员</a>&nbsp;&nbsp;&nbsp;<a href="/user/payment.act" class="btn_s bred">成为通资信通vip会员</a></s:if>
            	<s:else>
            		您的 <s:if test="#session._user.viptype == 1">
            			资信通认证会员
            		</s:if><s:else>
            			资信通vip会员
            		</s:else>
            		 将于 <s:date name="usrMemberPeroid.memberEnd"/> 到期 &nbsp;&nbsp;&nbsp;
            		 <a href="/user/payment!orderEdit.act?memberType=${session._user.viptype}">立即续费</a>
            		  <s:if test="#session._user.viptype == 1">&nbsp;&nbsp;&nbsp;
            		 	<a href="/user/payment!orderEdit.act?memberType=2">升级为vip</a>
            		 </s:if>
            	</s:else>
            	<!-- 
            	<s:if test="#session._user.viptype == 1"><a href="/user/payment.act" class="btn_s bred">成为通资信通vip会员</a></s:if><s:elseif test="#session._user.viptype == 0"><a href="/user/payment.act" class="btn_s bred">成为通资信通会员</a>&nbsp;&nbsp;&nbsp;<a href="/user/payment.act" class="btn_s bred">成为通资信通vip会员</a></s:elseif>
            	 -->
            </p>
    </div>
    <div class="clear"> &nbsp; </div>
  ...
  <div class="grid_710">
    ...
    <table class="gold-table" border="0" cellspacing="0" cellpadding="0">
	  <caption>
	  <span class="fr view_all"><a href="/service/serviceOrder!showSinOrdersByApplicant.act?mparamid=19">查看全部»</a></span>
	  <h6>服务申请</h6>
		  ...
		  <tr>
	       	  <td>
					...
					<s:if test="#item.itemTitle.length()>10">
   						<s:property value="#item.itemTitle.substring(0,10)"></s:property>
   					</s:if><s:else>
   						${item.itemTitle}
   					</s:else>
  				</label>
