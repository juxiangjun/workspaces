...
$(function() {
	...
	$("#loanForm").validate();
	$('#product_proIntroduce').xheditor({tools:'simple',skin:'nostyle'});
});
...
function interest(){
	...
	for(var i = limist; i<=limien;i++){
		...
		if(interTrObj.length <=0){
			var intetr = "<tr id='interTr" + i + "'><td>" + baserate_inteTextArr[i-1] + "</td><td>" + baserate_inteArr[i-1] + "%</td><td><input type='text' name='financeProductSpModel.rateDown' class='digits' id='rateDown" + i + "' size='2'/>";
			...
			sliderAdd(('slider-range' + i),('rateDown' + i),('rateUp' + i));
		}
	}
}
function sliderAdd(sliderId,rateDownId, rateId){
sliderId = "#" + sliderId;
rateId = "#" + rateId;
...
<div class="container_950 box_4">
	...
	<s:hidden name="financeProductSpModel.rateEnId" id="rateEnId"></s:hidden>
		...
		<div class="C_form">
	            <dl>
	              ...
	              <dd>
	                <h6>贷款类型：</h6>
	                <common:select name="product.financeType" style="width:200px;" id="product_financeType" valueSetId="32" onchange="finance_type_chenge();" cssClass="S_width"/>
	              </dd>
	              <dd>
	                <h6>贷款期限：</h6>
		...
		<div class="C_form">
			<dl>
	           <dd>
				...
				<dd>
	                <h6>贷款产品介绍：</h6>
	                <s:textarea name="product.proIntroduce" cols="80" rows="10" id="product_proIntroduce" cssClass="{maxlength:500}"></s:textarea>
	              </dd>
			</dl>
		</div>
		<div class="clear"></div>
			...
			<table border="0" cellspacing="0" cellpadding="0" style="width:100%" >
				...
				<s:iterator value="dataFileList" id="item">
					<tr>
						...
						<td>
							<input type="text" name="dataNum" id="dataNum${id }" class="digits" maxlength="2" value="1"/>
							<input type="hidden" name="dataMemo" id="dataMemo${id }"/>
						</td>
							<!-- 
						<td>
					...
					<s:if test="haveMemo == 1">
						<tr>
							<td colspan="4">
								<textarea name="otherMemo" id="otherMemo${id }" cols="90" rows="5" cssClass="{maxlength:200}"></textarea>
							</td>
						</tr>
					</s:if>
				</s:iterator>
		...
		<div class="C_form">
			<dl>
	              ...
	              <dd>
	                <h6>其他申请条件：</h6>
	                <s:textarea name="product.otherRequire" cols="60" rows="5" cssClass="{maxlength:500}" id="product_otherRequire" ></s:textarea>
	                <label for="product_otherRequire" class="error" generated="true" style="display:none;"></label>
	                <!-- if(this.value.length>10){alert('11');this.value=this.value.substring(0,10);} -->
	              </dd>
	              <dd>
	                <h6>备注：</h6>
	                <s:textarea name="product.memo" cols="60" rows="5" cssClass="{maxlength:500}" id="product_memo" ></s:textarea>
	                <label for="product_memo" class="error" generated="true" style="display:none;"></label>
	                <!-- if(this.value.length>10){alert('11');this.value=this.value.substring(0,10);} -->
	              </dd>
				</dl>
			</div>
		...
		<div class="C_form">
			<div style="font-size:12px;">
				...
				（如：项目1：营业面积    项目类型：输入框 ）<br/>
				<input type="button" value="添加自定义项" id="addDefineBut" class="btn_s blue2"/><input type="button" value="自定义项预览" onclick="previewExtends();" class="btn_s blue2"/><br/>
				<label id="extendsError" class="error">项目名与选项都是必填项！</label>
			</div>
			<div class="hr_10"> &nbsp; </div>
			<div class="box_4" style="width:100%;">
			...
			<!-- 
				<tr id="1">
					...
					<td class="view_detail01"><a href='javascript:deltr(1)' >删除</a></td>
				</tr>
				 -->
			</div>
		</div>	
			...
			<div class="hr_10"> &nbsp; </div>
				...
				<input type="button" value="预览产品" onclick="preview()" class="but_gray"/>
	...
	<div style="width:880px;height:200px; overflow-y:scroll; border:1px solid #ccc;padding-left:10px;margin:30px;"> 
	中国资金网融资产品发布规则<br/>
	<p>
	...
	<p>4.因您保管疏忽或因黑客行为、计算机病毒侵入或发作致使帐号、密码被他人非法使用、盗用、篡改或丢失的，本网站不承担责任。如您发现任何非法使用用户帐号或安全漏洞的情况，请立即与本网站联系。 
	</p>
	...
	<p>二、服务内容及费用 </p>
	<p>本网站的具体服务内容由本网站根据实际情况提供，例如融资、理财、增值服务等，本网站保留根据实际情况随时调整本网站平台提供的服务种类、形式。本网站不承担因业务调整给用户造成的损失。本网站对提供的服务拥有最终解释权。您因进行交易、向本网站获取有偿服务而发生的相关服务费用和所有应纳税赋，以及相关硬件、软件、通讯、网络服务及其他方面的费用均由您自行承担。本网站保留变更、中断或终止部分网络服务的权利。 
	</p>
	<p>1.涉及第三方网站 </p>
	<p>本网站的网站内容可能涉及由第三方所有、控制或者运营的其他网站的内容或链接（以下称“第三方网站”）。本网站不能保证也没有义务保证第三方网站上的信息的真实性和有效性。对于该等内容或链接，您确认按照第三方网站的服务协议确定相关权利义务，而不是按照本协议。第三方网站的内容、产品、广告和其他任何信息均由您自行判断并承担风险责任，而与本网站无关。 
	</p>
	<p>2.不保证 </p>
	<p>本网站提供的服务中不带有对本网站中的任何用户、任何交易的任何保证，无论是明示、默示或法定的，因此本网站及其股东、创建人、高级职员、董事、代理人、关联公司、母公司、子公司和雇员不保证由其他用户或机构提供的内容的真实性、充分性、及时性、可靠性、完整性和有效性，并且不承担任何由此引起的法律责任。 
	</p>
	<p>3.免责声明 </p>
	<p>在任何情况下，本网站对您使用本网站服务而产生的任何形式的直接或间接损失均不承担法律责任，包括但不限于资金损失、利润损失、交易中断损失等。因为本网站或者涉及的第三方网站的设备、系统问题或者因为计算机病毒造成的损失，本网站均不负责赔偿。如果因为用户违反有关法律或者本协议之规定，使本网站遭受任何损失，受到任何第三方的索赔，或任何行政管理部门的处罚，用户应对本网站作出相应的补偿。 
	</p>
	<p>4.服务变更、中断或终止 
	</p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（1）如因系统维护或升级的需要而需暂停网络服务、服务功能的调整，本网站将尽可能在网站上进行通告。由于用户未能及时查看、浏览通告信息而造成的损失，本网站不承担任何责任。 
	</p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（2）用户同意，本网站可自行决定变更提供服务的内容，本网站可自行决定授权第三方向用户提供原本由本网站提供的服务，不需向用户承担任何因变更服务所产生的责任。 
	</p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（3）如发生下列任何一种情形，本网站有权单方面中断或终止向用户提供服务而无需通知用户，无需对用户或第三方承担任何责任： 
	</p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;（3-1）用户提供的个人资料不真实； </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;（3-2）用户违反本服务条款； </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;（3-3）未经本网站同意，将本网站平台用于商业目的。 </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;（3-4）用户可随时根据需要通知本网站终止提供服务，用户服务终止后，用户使用服务的权利同时终止。自用户终止之日起，本网站与用户有关服务协议约定的权利义务不再履行。 </p>
	<p>三、服务协议的完善和修改 </p>
	...
	<p>3.特别约定 </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（1）用户使用本服务的行为若有任何违反国家法律法规或侵犯任何第三方的合法权益的情形时，本网站有权直接删除该等违反规定之信息，并可以暂停或终止向该用户提供服务。 </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（2）若用户利用本服务从事任何违法或侵权行为，由用户自行承担全部责任，因此给本网站或任何第三方造成任何损失，用户应负责全额赔偿。并使本网站免受任何损害赔偿。 </p>
	<p>&nbsp;&nbsp;&nbsp;&nbsp;（3）所有发给用户的通告或通知都可以通过重要页面的公告、通告、电子邮件以及常规信件的形式传送。 </p>
	<p>四、本服务协议最终解释权归本网站所有。 </p>
	<p>五、适用法律和管辖 </p>
