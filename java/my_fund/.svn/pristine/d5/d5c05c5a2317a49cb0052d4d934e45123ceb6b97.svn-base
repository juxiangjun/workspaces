...
function doStatus(){
	try{
		...
		if(st && et && ""!=st && ""!=et){
			var temp = new Date();
			var s = new Date(st);
			var c = new Date(temp.getFullYear()+"-"+(temp.getMonth()+1)+"-"+temp.getDate());
			var e = new Date(et);
			if(c.getTime()<s.getTime()){
				  $("#sell_c").html("&nbsp;&nbsp;未开始销售");
			}else if(c.getTime()>=s.getTime() && c.getTime()<=e.getTime()){
				  $("#sell_c").html("&nbsp;&nbsp;正在销售中");
			}else if(c.getTime()>e.getTime()){
				  $("#sell_c").html("&nbsp;&nbsp;已结束销售");
			}else{
				  $("#sell_c").html("");
			}
  	...
  	<div class="M_out_L">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0" class="Search_table">
          ...
          <tr>
            ...
            <td class="Search_t_connect01" id="sell_s"><s:date name="prdBankfinance.sellstart"/></td>
            <td class="Search_t_connect">销售终止日期</td>
            <td class="Search_t_connect01" id="sell_e"><s:date name="prdBankfinance.sellend"/></td>
          </tr>
          <tr>
            <td class="Search_t_connect">付息周期</td>
