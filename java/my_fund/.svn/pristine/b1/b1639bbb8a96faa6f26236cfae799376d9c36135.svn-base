...
public class OrdCheckLogDAOImpl extends HibernateDAO<OrdCheckLog, Integer> implements OrdCheckLogDAO {
	...
	public String getSuggest(Integer appId, Integer cp) {
		String hql = "from OrdCheckLog t where t.ordId=:applyId and t.checkContent=:cp order by id desc";
		Hashtable<String, Object> params = new Hashtable<String, Object>();
		params.put("applyId", appId);
		params.put("cp", cp);
		List<OrdCheckLog> l = list(hql, params);
		if(l!=null && l.size()>0){
			return l.get(0).getCheckView();
		}
		return "";
	}
}
