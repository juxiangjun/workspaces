	...
	<table class="M_box">
		...
		<s:iterator value="recommendProducts">
			<tr class="top_color01">
				<td style="text-align: center;"><input type="checkbox" name="checkId" value="${id }_${financeName }"/></td>
				<td style="text-align: center;">${financeNum }</td>
		...
		<tr>
			<td colspan="8" style="padding-bottom:5px; padding-top:3px;" style="border-left:1px solid #d7d7d7; border-bottom:1px solid #d7d7d7;">
				<input type="button" onclick="SelectRecom()" value="选择" class="but_gray" />&nbsp;
				<input type="button" name="cancel" value="取消" onclick="$('#pro_recommend').dialog('close');" class="but_gray"/> 
			</td>
		</tr>
	</table>
</div>