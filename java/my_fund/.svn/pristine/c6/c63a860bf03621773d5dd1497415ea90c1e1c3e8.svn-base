...
//		System.out.println(username);
	...
	private String commRegister(String succStr,String failStr){
		...
		if(reference != null && reference.trim().length() > 0){			
			user.setReferenceId(Integer.valueOf(reference));
		}
		
		RegisterUserSrModel srModel = accountService.addRegisterUser(user, request.getRemoteAddr());
		msg = srModel.getMsg();
		if(srModel.getUser() == null){
			return failStr;
		}else{
			context.getSession().put("_user", srModel.getUser());
