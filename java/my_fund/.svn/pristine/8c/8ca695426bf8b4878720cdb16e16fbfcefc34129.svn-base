	...
	<form id="searchForm" action="/service/supplyInfoAction!home.act" method="post">
		<div class="fl">
			...
			<span style="padding-right:20px;">
			    	...
			    	<option value="其他行业" <s:if test="query[0] == '其他行业'">selected</s:if>>其他行业</option>
			   		</select>&nbsp;&nbsp;&nbsp;&nbsp;		
			</span>
			<span>融资方式：&nbsp;&nbsp;</span>
			<span style="padding-right:20px;">
		...
		<tr <s:if test="#st.even"> style="${style}" </s:if>>
			...
			<td>
		    	<s:iterator id="parameter" value="investStyleList">
    				<s:if test="#parameter.code.trim()==#item.investType.trim()">
    					${parameter.name }
    				</s:if>
    			</s:iterator>  
		    	<s:if test="#item.investType==null or #item.investType==''">
		    		不限
		    	</s:if>  
			</td>
			<td>
