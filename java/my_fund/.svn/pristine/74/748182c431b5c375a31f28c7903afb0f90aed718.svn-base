...
//	}
	...
	public String updateFinancePage() {
		...
		UsrUser user = accountService.getUserById(product.getUserId());
		userGroup = this.dictoryDataService.getDicUserTypeById(user.getType()).getGroup();
...
//		financeProductSpModel.setUserId(user.getId());
		...
		UsrUser user1 = accountService.getUserById(product.getUserId());
		userName = user1.getUsername();
		companyName = dictoryDataService.getCompanyNameByUserId(user1.getId());
