...
<script language="javascript">
    ...
    function downloadAttach(attachid){
    	var url = "/user/loan/financeAttach!viewAttach.act";
    	var param = {'attachId':attachid};
    	//window.location= url + "?attachId=" + attachid;
    	$.post(url,param, function(data){
    	//alert(data);
    ...
    function downloadTemplate(prdId){
    	var url = "/user/loan/financeAttach!downTemplate.act";
    	var param = {'attachId':prdId};
    	$.post(url,param, function(data){
    		var da = $('#downloadform');
    		if(da.length == 0){
    			da = $("<div id='downloadform'></div>");
    ...
    function viewAgreeAttach(){
    	var url = "financeAttach!applyAgree.act";
    	var param = {'apply.id':'${apply.id}'};
    	//window.location= url + "?attachId=" + attachid;
    	$.post(url,param, function(data){
    		$('#agreeAttach').html(data);
    		$('#agreeAttach').dialog({width:400,modal:true});
    	}, 'html');
...
<!--头部结束-->
  <!--理财服务产品列表页面开始-->
	<div class="M_menu">
		...
		<a href="/user/loan/userApplyManag!viewFinanceApply.act?apply.id=${apply.id }">融资申请查看</a>&nbsp;&nbsp;<b>></b>&nbsp;&nbsp;提交资料&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#97181d;">${msg }</span>
	</div>
	<div class="hr_10"> &nbsp; </div> 
          ...
          <tbody>
           	<tr class="top_color01">
	           	...
	           	<td id="attStatus0" style="padding:5px 0;">
		   			...
		   			<s:if test="apply.agreeUploadWay == 1">
		   				<br/>快递公司：${apply.agreeMemo }<br/>
	   					快递单号：${apply.agreeFileUpload }
		   			</s:if>
	           	</td>
	           	<td align="right" style="padding-right:25px;">
   					<s:if test="apply.agreeStatus == 212 || apply.agreeStatus == 213 || apply.agreeStatus == 215">
			   			<span id="operatOnline0"><input type="button" class="bgray hid" value="网上提交" onclick="uploadFile(0,1);"/></span>
		   			</s:if>
   					<s:if test="apply.agreeStatus == 212 || apply.agreeStatus == 215 || apply.agreeStatus == 218">
   						<s:if test="apply.agreeUploadWay != null && apply.agreeUploadWay == 1">
   						...
   						<s:else>
   							<span id="operatOffline0"><input type="button" class="bgray hid" value="线下邮寄" onclick="underDiv(0,1,'','')"/></span>
   						</s:else>
		   			</s:if>
	           	</td>
           	</tr>
          <% int num = 1; %>
			...
			<tr class="top_color01">
				<td style="padding:5px 25px 5px 20px;">
						<%=num++ %>.&nbsp;${item.dataName }
						${item.supplyName }
						<s:if test="haveTemplate != null && haveTemplate == 1">
							<a href="javascript:downloadTemplate('${item.id}');" class="red">模版下载</a>
						</s:if>
				...
				<td>
					<s:if test="supplyWay.indexOf('175') >=0 && supplyWay.indexOf('176') >=0">
						网上提交或线下邮寄
					</s:if>
					<s:else>
	   			...
	   			<td id="attStatus${id }" style="padding:5px 0;">
	   				...
	   				<s:if test="uploadStatus != 212 && uploadWay == 1">
	   					<br/>快递公司：${attachMemo }<br/>
	   					快递单号：${postid }
	   				</s:if>
	   				<s:if test="uploadStatus != 212 && uploadWay == 0">
	   			...
	   			<td align="right" style="padding-right:25px;" >
	   				...
	   				<s:if test="supplyWay == '' || supplyWay.indexOf('176') >=0">
	   					<s:if test="(apply.applyStatus==302 || apply.applyStatus==308) && (uploadStatus == 212 || uploadStatus == 215 || uploadStatus == 218)">
	   						<s:if test="uploadWay != null && uploadWay == 1">
					   			<span id="operatOffline${id }"><input type="button" class="bgray hid" value="线下邮寄" onclick="underDiv('${item.id}',0,'${attachMemo }','${postid }')"/></span>
	   						</s:if>
	   						<s:else>
	   							<span id="operatOffline${id }"><input type="button" class="bgray hid" value="线下邮寄" onclick="underDiv('${item.id}',0,'','')"/></span>
	   						</s:else>
			   			</s:if>
	   				</s:if>
	   			</td>
