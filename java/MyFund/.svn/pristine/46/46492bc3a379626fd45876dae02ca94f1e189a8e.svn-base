	...
	function(event) {
		...
		for (i = 0; i < allCountys.length; i++) {
			if (v.toUpperCase() === allCountys[i].pinYinChar.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:' provinceId=" + allCountys[i].provinceid + " cityId=" + allCountys[i].cityId + " countyId=" + allCountys[i].id + ">" + allCountys[i].cityName + "-" + allCountys[i].name + " (<span style='color:red'>" + v.toUpperCase() + "</span>" + allCountys[i].pinYinChar.substring(v.length, allCountys[i].pinYinChar.length) + ")</a></li>";
				if (htmlArr.length > 9) {
					break;
					return false;
				}
				autoWidth = autoWidth < (allCountys[i].cityName + allCountys[i].name + allCountys[i].pinYinChar).length ? (allCountys[i].cityName + allCountys[i].name + allCountys[i].pinYinChar).length: autoWidth;
				continue;
			};
			if (v === allCountys[i].name.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:' provinceId=" + allCountys[i].provinceid + " cityId=" + allCountys[i].cityId + " countyId=" + allCountys[i].id + ">" + allCountys[i].cityName + "-" + "<span style='color:red'>" + v + "</span>" + allCountys[i].name.substring(v.length, allCountys[i].name.length) + " (" + allCountys[i].pinYinChar + ")</a></li>";
				if (htmlArr.length > 9) {
					break;
					return false;
				}
			...
			if (v.toLowerCase() === allCountys[i].pinYin.substring(0, v.length)) {
				htmlArr[htmlArr.length] = "<li><a class='allcityClass' href='javascript:' provinceId=" + allCountys[i].provinceid + " cityId=" + allCountys[i].cityId + " countyId=" + allCountys[i].id + ">" + allCountys[i].cityName + "-" + allCountys[i].name + " (<span style='color:red'>" + v.toLowerCase() + "</span>" + allCountys[i].pinYin.substring(v.length, allCountys[i].pinYin.length) + ")</a></li>"
				if (htmlArr.length > 9) {
					break;
					return false;
				}
				autoWidth = autoWidth < (allCountys[i].cityName + allCountys[i].name + allCountys[i].pinYin).length ? (allCountys[i].cityName + allCountys[i].name + allCountys[i].pinYin).length: autoWidth;
				continue;
			};
		}
	...
	function() {
		...
		for (i = 0; i < allProvinces.length; i++) {
			if (allProvinces[i].id == provinceId) {
				provinceName = allProvinces[i].name;
				pid = allProvinces[i].id
			};
		}
		for (i = 0; i < allCitys.cities.length; i++) {
			if (allCitys.cities[i].id == cityId) {
				cityName = allCitys.cities[i].name;
				cid = allCitys.cities[i].id;
			}
		}
		if (currentClass == "proCityQueryAll") {
			...
			$("body").data("pAllId", pid);
			$("body").data("nameOfCityAll", cityName);
			$("body").data("cAllId", cid);
		}
...
function viewProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		var p_name = provinces[i].name;
		if (provinces[i].name == '���ɹ�������') {
			p_name = '���ɹ�';
		} else if (provinces[i].name == '����ʡ') {
			p_name = '����';
		} else {
			p_name = provinces[i].name.substr(0, 2);
		}
...
function viewCities(i) {
	...
	$("body").data("pName", provinces[i].name);
	$("body").data("pId", proId);
	citys = [];
	var j = 0;
	$.each(cities,
...
//	$("#star").val(nameOfProvince + "-" + cityCurName);
...
function viewAllProvince(page) {
	...
	for (var i = start; i < end; i++) {
		...
		if (allProvinces[i].name == '���ɹ�������') {
			p_name = '���ɹ�';
		} else if (allProvinces[i].name == '����ʡ') {
			p_name = '����';
		} else {
...
function allCountyPage(page) {
	...
	var pid = $("body").data("pAllId");
	var cid = $("body").data("cAllId");
