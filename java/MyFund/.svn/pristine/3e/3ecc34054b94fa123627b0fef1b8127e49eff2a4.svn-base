...
<script language="javascript">
	...
	function toPage(pageNum, pageSize){
		...
		$("#erro").html("");
		var url = "/user/loan/LoanRequest.act?pager.currentPage=" + pageNum + "&pager.pageCount=" + pageSize;
...
<!--头部结束-->
  ...
  <form id="frmLoanManage" action="/user/loan/LoanRequest.act" method="post">
  ...
  <div class="hr_10"> &nbsp;</div>
       <table class="container_950 box_4 ctl">
	      <tr class="top_color">
	        <td width="14%" style="padding-left:20px;">融资意向编号</td>
	        <td width="12%">发布时间</td>
	        <td width="12%">需求金额</td>	        	        
	        <td width="15%">用途</td>
	        <td width="12%">期限</td>	
	        <td width="12%">可接受年利率</td>	                
	        <td width="10%">状态</td>
			...
			<tr class="gold_connect">
				<td style="padding-left:20px;">${item.applyNum}</td>
	   			...
	   			<td align="right">
		   			<div class="view_detail" style="padding:5px 20px 5px 0;">
			   			<a href="/user/loan/LoanRequest!loanManageDetail.act?loan.id=${item.id }">查看</a>		
		   			</div>     
	   				<!--  <a href="/user/loan/LoanRequest!loanManageDetail.act?loan.id=${item.id }">查看</a><br/>
			 ...
			 <tr class="top_color">
		        <td style="padding-left:22px; font-size:14px; " colspan="8">
