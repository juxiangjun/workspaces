...
function payCost(id){
	if(window.confirm("是否确定付款信息")){
		if(id!=null && id!=0){
			$.post("/admin/payment/manage!payCost.act",{ordId:id},function(a){
				if(a=='success'){
					$("#umsg").removeClass("errormsg");
...
<div id="hld">
	<div class="wrapper">
		...
		<div class="block">
		    <div id="umsg" class="message errormsg" style="display: none;"></div>
			<s:if test="msg!=null"><div class="message">
			<p>${msg}</p>
			</div></s:if>
			...
			<div class="block_content">
				<form id="searchform" action="/admin/payment/manage.act" method="post">
				...
				<table cellpadding="0" cellspacing="0" width="100%">
						<tr>
							...
							<td>
								<label>付款状态</label><s:select name="ordStatus" list="#{'-1':'--付款状态--',0:'未付款',1:'已付款'}" listKey="key" listValue="value"/>
								<!-- 
						...
						<tr>
							<td>
								<label>订单编号</label><input value="${ordNum }" type="text" name="ordNum" class="text" style="width: 100px;"/>
							</td>
							<td>
								<label>订单状态</label><s:select name="cancelled" list="#{'-1':'--订单状态--',0:'正常',1:'取消'}" listKey="key" listValue="value"/>
							</td>
							<td>
								<!-- 
