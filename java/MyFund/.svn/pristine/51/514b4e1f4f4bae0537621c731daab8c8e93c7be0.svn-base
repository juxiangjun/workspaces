...
public class UsrUserDAOImpl extends HibernateDAO<UsrUser, Integer> implements UsrUserDAO{
	...
	public Pager findPartnerUser(FindPartnerUserSpModel spModel){
		...
		hql.append("from UsrUser as model where model.status=0 and model.referenceId=:referenceId");
		Hashtable<String, Object> paramMap = new Hashtable<String, Object>();
		paramMap.put("referenceId", spModel.getPnrUserId());
		if(spModel.getStartDt() != null){
			hql.append(" and date(model.createdt) >= :appstartdt");
			paramMap.put("appstartdt", spModel.getStartDt());
		}
		if(spModel.getEndDt() != null){
			hql.append(" and date(model.createdt) <= :appenddt");
			paramMap.put("appenddt", spModel.getEndDt());
		}
		if(spModel.getUserType() != null && !spModel.getUserType().equals("-1")){
			if(spModel.getUserType().split("-").length>1){
				String[] types=spModel.getUserType().split("-");
				hql.append(" and model.type>=:userType1 and model.type<=:userType2");
				paramMap.put("userType1", Short.valueOf(types[0]));
				paramMap.put("userType2", Short.valueOf(types[1]));
			}else{
				hql.append(" and model.type=:userType");
				paramMap.put("userType", Short.valueOf(spModel.getUserType()));
			}
		}
		hql.append(" order by model.id desc");
