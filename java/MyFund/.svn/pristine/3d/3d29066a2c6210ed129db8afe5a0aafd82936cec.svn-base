...
function prdTemplate(prdId){
	var url = "/admin/loan/prdAttach!applyDataList.act";
			...
			<s:hidden name="product.proStatus" id="product_prostatus"></s:hidden>
				<table cellpadding="0" cellspacing="0" width="100%" class="tablesorter">
					<tbody>
						...
						<tr><td>贷款期限</td><td>${product.financeLittledt } ~ ${product.financeMostdt }月</td></tr>
                	...
                	<s:if test="productAreaList.size()==0">全国</s:if><s:else><s:iterator value="productAreaList" id="item">${item.name }&nbsp;</s:iterator></s:else></td></tr>
						...
						<tr class="finance_type_151"><td>企业的总资产</td><td><common:print valueId="product.companyAllAsset" valueSetMap="ZJ112"/></td></tr>
						...
						<tr><td>备注</td><td>${product.memo }</td></tr>
						...
						<input type="button" value="上传申贷资料模版" onclick="prdTemplate('${product.id }');"/>
						<s:if test="product.attach != null && product.attach != ''">
				   			<input type="button" value="下载产品文档" onclick="window.open('/admin/loan/prdAttachDown.act?prdId=${product.id }');"/>
			   			</s:if>
						</td></tr>
					</tbody>
				</table>
