    ...
    <div class="user_login01_content">
       <div class="on">
          <table>
            ...
            <tr><td align="center"><input type="button" onclick="javascript:vershow();" class="btnsub bred" style="width:90px;" value="立即验证  " />
            <input type="button" onclick="cancelCheck(${product.id})" class="btnsub bred" style="width:90px;" value="跳过验证" />
            </td></tr>
            </table>
        </div>
	  </div>
       ...
       <div class="on">
          <table>
            <tr><td><h6>验证码：</h6><input id="verifycode" type="text" name="verifycode" class="input-text" size="20" />
          	<input id="sendmobile" type="button" class="btnsub bred" onclick="sendMobileMsg();" value="点击发送验证码（1分钟内只能发送一次）"/>
            </td></tr>
   	  		<tr><td><span id="mobileMsg" style="color: red">提示：验证手机可以使您用中国资金网的各项功能</span></td></tr>
            <tr><td><h6>&nbsp;</h6><input type="button" onclick="javascript:verMobile(${product.id});" class="btnsub bred" style="width:90px;" value="确定" /></td></tr>
            </table>
        </div>
	  </div>
	</div>
...
<div class="M_out">
  	<div class="M_out_L">
        ...
        <table border="0" cellspacing="0" cellpadding="0" class="Search_table">
          ...
          <tr>
            <td class="Search_t_connect">融资渠道</td>
            <td class="Search_t_connect01" colspan="3">
            	<s:if test="product.orgName.indexOf(' ') >= 0">
					<s:property value="product.orgName.substring(0,product.orgName.indexOf(' '))"></s:property>
				</s:if>
				<s:else>
					${product.orgName }
				</s:else>
						...
						<tr>
							...
							<td width="33%" align="center">
		                    	<s:if test="product.interestType == 155">
		                    		<span><img src="/images/ico_upward.png" style="float:left;margin:5px 3px 0 60px;"/></span><span style="float:left;">${rateDown }%-${rateUp }%</span>
		                    	</s:if>
		                    	<s:else>
		                    		视具体情况而定
		                    	</s:else>
  ...
  <!--融资服务搜索结果页产品详情页面结束--> 
    ...
    <div class="hr_10"> &nbsp; </div>
      ...
      <div class="box_3 " style="padding-left:5px; padding-top:15px; padding-bottom:15px;line-height:30px;"  >
    	<div style="width : 230px; " >
    		<form action="/loan/loanCalculator.act" method="post">
				<table class="C_table">
					<tr>
						<td colspan="2" align="center">贷款计算器</td>
					</tr>
					<tr>
						<td class="c-left">贷款金额：</td>
						<td><s:textfield id="edLastSum" cssClass="txtnum" name="edLastSum" size="13"></s:textfield>&nbsp;元</td>
					</tr>
					<tr>
						<td class="c-left">贷款期限：</td>
							<td><s:textfield onblur="AutoShowRate();" id="edTimes" cssClass="txtnum" name="edTimes" size="13"></s:textfield>&nbsp;月</td>
					</tr>
					<tr>
						<td class="c-left">年利率：</td>
							<td><s:textfield id="edRate" cssClass="txtnum" name="edRate" size="13"></s:textfield>&nbsp;%</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2" align="center">
							<input type="submit" value="计算" class="btnsub bred" ></input>
						</td>
					</tr>
				</table>
			</form>
