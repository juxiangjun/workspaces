...
public class OrdLoanRequestDAOImpl extends HibernateDAO<OrdLoanRequest, Integer> implements OrdLoanRequestDAO {
	...
	public Pager findReqByCon(Integer currentPage, Integer pageCount, RequestSearch rq, Integer type) {
		...
		String hql = "select o from OrdLoanRequest o, OrdLoanAllocation t where o.id=t.requestId and t.orgId=:orgId";
		params.put("orgId", rq.getOrgId());
		if (rq != null) {
			// 需求编号
	...
	public Pager getAdminRequest(FindFinanceAdminSpModel spModel) {
		...
		Hashtable<String, Object> params = new Hashtable<String, Object>();
		if (spModel.getRequestNum() != null && spModel.getRequestNum().trim().length() > 0) {
			hql.append(" and t.applyNum = :applyNum");
			params.put("applyNum", spModel.getRequestNum());
		}
	...
	public List<OrdLoanRequest> getTopThree(int userId) {
		String hql = "from OrdLoanRequest t where t.isdeleted=0 and t.userId = :userId order by t.id desc";
		Hashtable<String, Object> params = new Hashtable<String, Object>();
		params.put("userId", userId);
		return findTopRows(hql, 3, params);
	}
...
//		String hql = "from OrdLoanRequest t where t.isdeleted=0 and t.applyStatus = 145 and t.loanOrganization like :userType";
