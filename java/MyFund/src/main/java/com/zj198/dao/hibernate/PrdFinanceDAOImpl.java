...
//		String hql = "from PrdFinance t where t.proStatus in (197,198,199) order by t.id desc";
	...
	public List<PrdFinance> recommendPrd(RecommendPrdSpModel spModel){
		String hql = "from PrdFinance o where o.proStatus = 198";
		Hashtable<String, Object> params = new Hashtable<String, Object>();
		if (spModel.getFinanceType() != null && spModel.getFinanceType() > 0) {
			hql = hql + " and o.financeType = :financeType";
			params.put("financeType", spModel.getFinanceType());
		}
		if (spModel.getUserId() != null) {
			hql = hql + " and o.userId =:userId";
			params.put("userId",  spModel.getUserId());
		}
		if (spModel.getPrdName() != null && spModel.getPrdName().length() > 0) {
			hql = hql + " and o.financeName like :financeName";
			params.put("financeName",  "%" + spModel.getPrdName() +  "%");
		}
		if (spModel.getLoanAmount() != null && spModel.getLoanAmount() > 0) {
			hql = hql + " and o.financeLittleamount <= :financeLittleamount and o.financeBigamount >= :financeBigamount";
			params.put("financeLittleamount", spModel.getLoanAmount());
			params.put("financeBigamount", spModel.getLoanAmount());
		}
		if (spModel.getLoanMonth() != null && spModel.getLoanMonth() > 0) {
			hql = hql + " and o.financeLittledt <= :financeLittledt and o.financeMostdt >= :financeMostdt";
			params.put("financeLittledt", spModel.getLoanMonth());
			params.put("financeMostdt", spModel.getLoanMonth());
		}
		if (spModel.getOrgName() != null && spModel.getOrgName().length() > 0) {
			hql = hql + " and o.orgName like :orgName";
			params.put("orgName", "%" + spModel.getOrgName() + "%");
//			hql = hql + " and (exists (select u.id from UsrBank u where o.userId = u.id and u.detailname like :bankname) "
//					+ " or exists (select u.id from UsrFinanceorg u where o.userId = u.id and u.orgname like :orgName))";
//			params.put("bankname", "%," + spModel.getOrgName() + ",%");
//			params.put("orgName", "%," + spModel.getOrgName() + ",%");
		}
		if (!StringUtils.isBlank(spModel.getOrgId())) {
			hql = hql + " and exists (select u.id from UsrUser u where o.userId = u.id and (";
			String[] opts = spModel.getOrgId().split(",");
