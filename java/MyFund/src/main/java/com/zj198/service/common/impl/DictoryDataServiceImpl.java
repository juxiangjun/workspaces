...
public class DictoryDataServiceImpl implements DictoryDataService {
	...
	public String getCompanyNameByUserId(Integer userId){
		...
		}else if(userType.getGroup() == Constants.USERTYPE_GROUP_PERSONAL){
			companyName = user.getRealname();
		}
		return companyName;
	}
	...
	@Override
	public void updateBaseRate(DicBaseRate dbr) {
		dicBaseRateDAO.update(dbr);
		
	}

	@Override
	public DicBaseRate findBaseRateById(Integer id) {
		return dicBaseRateDAO.get(id);
	}
