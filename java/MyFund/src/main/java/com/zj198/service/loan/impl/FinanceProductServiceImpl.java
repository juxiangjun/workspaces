...
//				}
	...
	public FinanceProductSrModel updateFinance(FinanceProductSpModel spModel) {
		...
		product.setMemo(newpro.getMemo());
		...
		product.setServiceCost(newpro.getServiceCost());
...
//				interest.setInterestBig(rateup);
		...
		if(product.getLogo() == null || product.getLogo().trim().equals("")){
			UsrUser user = usrUserDAO.get(product.getUserId());
			product.setLogo(user.getLogo());
		}
		if(product.getOrgName() == null || product.getOrgName().trim().equals("")){
			product.setOrgName(this.dictoryDataService.getCompanyNameByUserId(product.getUserId()));//保存银行名称
		}
...
//		}
		...
		product.setLogo(user.getLogo());//产品logo
		product.setOrgName(this.dictoryDataService.getCompanyNameByUserId(user.getId()));//保存银行名称
...
//					df.setOtherMemo(f[4]);
 ...
 String needEnsure, String operatIncome, Integer settingYear, String bankSalaryList, String repaymentType, Integer runningArea, Integer industryId, Integer childId, Integer selectArea) {
	...
	public List<PrdFinance> recommendPrd(RecommendPrdSpModel spModel){
		return prdFinanceDAO.recommendPrd(spModel);
	}
