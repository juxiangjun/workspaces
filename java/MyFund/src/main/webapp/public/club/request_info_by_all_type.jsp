...
<jsp:include page="/public/head1.jsp"/>
  ...
  <div class="hr_10"> &nbsp; </div>
  <div class="M_menu">
  	<a href="/club/clubAction.act">融资俱乐部</a>&nbsp;&nbsp;<b>&gt;</b>&nbsp;&nbsp;项目信息库
  </div>
  <div class="hr_10"> &nbsp; </div>
  <div style="width:950px; margin:0 auto;">
	  ...
	  <div class="grid_350 box_4" style="overflow:hidden;">       
        <div class="blue_bg">
          <div class="gray news_menu tab_menu2">
            <ul class="mews_ul">
              <li class="hover">找项目</li>
            </ul>
          </div>			         
          <div>
          	<div class="xxkbox2 first2">
          	<!-- 表单开始 -->
	            <form class="box_form" action="/service/requestInfoAction!home.act" method="post" id="frmQueryRequestInfo" style="margin-left:10px;">
			  ...
			  <!-- 表单结束 -->          	
				  <s:if test="requestInfoList.size>0">
            		<ul style="float:left;margin-right:50px;width:260px; padding-left:7px;"  class="new_li">
						<s:iterator id="item" value="requestInfoListForQueryPanel" status="st">
							<li title="${item.title}">
	...
	<div style="width:310px; float:right;"><a href="/service/supplyInfoAction4User!editFirstStep.act"><img src="/images/fbxx_zj.jpg" /></a></div>
	<div class="hr_10">&nbsp;</div>
	<!-- 资金信息开始 -->
	<div class="box_4" style="width:950px; overflow:hidden; margin-right:10px;">
	  <div class="menu_gray" style="border-bottom:4px solid #97181d;">
	 	<span class="fr"><a href="/service/requestInfoAction!home.act">更多»</a></span>
	    <h6 style="font-weight:bolder">项目信息</h6>
	  </div>	  
	</div>
	<div class="hr_10">&nbsp;</div>
	<!-- 1 -->
	...
	<s:iterator id="item" value="industryList" status="st">
		<s:if test="#st.index>0 && (#st.index+1) % 3 ==0">
		    ...
		    <div class="sky_blue"><span class="fr"><a href="javascript:doQueryRequestInfo('${item.name }')">更多»</a></span>
		      <h6>${item.name } </h6>
		    </div>
		    <div class="box_3">
		      <ul class="sky_new_li">
		        <s:set name="count" value="0"></s:set>
		      	<s:iterator id="requestInfo" value="requestInfoList">
		    	    <s:if test="#item.name==#requestInfo.industry">
		    	     <s:set name="count" value="#count+1"></s:set>
		    	   	 <li><a href="/service/requestInfoAction!detail.act?clbRequestInfoId=${requestInfo.id }">${requestInfo.title}</a></li>
		    		</s:if>
		        </s:iterator>
		        <s:if test="#count<1">
		        	<li>无记录</li>
