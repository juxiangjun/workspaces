...
//通用选项卡
	...
	function toPage(pageNum, pageSize) {
		...
		var url = "";
		if(j=="-1"){
			url = "/user/message!viewMsg.act?pager.currentPage=" + pageNum + "&pager.pageCount=" + pageSize;
		}else if(j=="0"){
			url = "/user/message!unRead.act?pager.currentPage=" + pageNum + "&pager.pageCount=" + pageSize;
		}else{
			url = "/user/message!read.act?pager.currentPage=" + pageNum + "&pager.pageCount=" + pageSize;
		}
...
<body><input type="hidden" id="css" value="${css }" name="css"/>
<!--头部-->
	  ...
	  <form id="comForm" action="" name="" method="post">
		<table class="n_table">
		  ...
		  <!-- 
		   ...
		   -->
		    <td><a href="javascript:void(0);" onclick="detail('${j.contents}',${j.id });"><s:if test="isRd==0"><font id="title${j.id }"  class="titl">${j.title }</font></s:if><s:else><font>${j.title }</font></s:else> </a></td>
		    <td><s:date name="#j.senddt " /></td>
		    <td>&nbsp;</td>
		    <td>&nbsp;</td>
