...
def get4SStoreInfoArray(data):
    data = data.lstrip()
    result = []
    hasBlank = data.count(" ")>0
    while hasBlank:
        item = format4SString(data)
        data = data.replace(item, "").lstrip()
        result.append(item)
        hasBlank = data.count(" ")>0
    return result
