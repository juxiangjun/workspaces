...
from utils.app_util import print_list,divide_by_record, is_null, get4SStoreInfoArray
...
class DataHandler:
    ...
    def do_second_step(self, config, node, data):   
        ...
        i = 0
        for item in result:
            data = BeautifulSoup(item).get_text()
            data =data.replace("4S","").replace("\n","").replace("\r","").lstrip()
            result[i] = get4SStoreInfoArray(data)  
            i = i+1
