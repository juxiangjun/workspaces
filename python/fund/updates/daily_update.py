...
class DailyUpdate:
	...
	def run(self):
		...
		for site in sites:
			...
			daily_nodes = config.get('update_nodes','daily').split(',')
			try:	
				for fund in funds:
					if 'roi' in daily_nodes:
						self.update_roi(config, fund, fetcher)

					if 'nav' in daily_nodes:
	...
	def update_nav(self, config, site, fund, fetcher):
		...
		url = config.get(node, 'url')
		url = url.replace('{fund_code}', fund.code)
		url = url.replace('{start_date}', get_offset_day(i)) 
		url = url.replace('{end_date}', get_offset_day(i+1))
		data = fetcher.get(url)
